...
module.exports = {
  ...
  'File field allows deselecting a file': function (browser) {
    browser.options.desiredCapabilities.name = 'File field allows deselecting a file'
    browser.expect.element('#file-example-fs .file-clear-btn').to.be.present
    browser.click('#file-example-fs .file-clear-btn')
    browser.expect.element('#file-example-fs div.custom-file > label').text.to.match(/\s*/)
    browser.expect.element('#file-example-fs div.vf-field-dirty.vf-field-valid.vf-field-touched').to.be.present
    browser.end()
  }
